...
const ZHIPU_AI_CONFIG = {
  ...
  model: process.env.ZHIPU_AI_MODEL || 'glm-5', // 智谱GLM模型
  baseUrl: process.env.ZHIPU_AI_BASE_URL || 'https://open.bigmodel.cn/api/paas/v4', // 智谱API海外站点
}
...
export async function callAliyunAI(
  ...
  const response = await fetch(`${ZHIPU_AI_CONFIG.baseUrl}/chat/completions`, {
    method: 'POST',
    headers: {
